...
**D2 — Ordonnancement par le planning (amendé, review PR #4).** Une entrée `scheduledTasks` par tâche : `apidae:sync` à 05:00, `deploy` à 05:30 — pas de tâche orchestratrice. L'ordre est garanti par la marge horaire (la sync d'ot-pertuis se compte en minutes) et l'indépendance des entrées garantit mécaniquement que le deploy part même si la sync échoue. Le travers legacy était l'ordre INVERSÉ (deploy 00:30 avant sync 05:30), pas la marge. La procédure tRPC `organization.deploy` appelle le même service que la tâche. *Alternative écartée (initialement retenue puis simplifiée en review) : tâche orchestratrice `jobs:daily` enchaînant les deux runTask — de l'indirection sans bénéfice à cette échelle.*
