...
**D2 — Ordonnancement iso legacy (amendé 2×, review PR #4).** Une entrée `scheduledTasks` par tâche — pas de tâche orchestratrice — aux **horaires legacy reproduits** (décision Quentin 29/07) : `deploy` 00:30, `apidae:sync` 05:30. Le contenu APIDAE du jour n'est donc visible qu'au build du lendemain — c'est le comportement de production que les clients connaissent, et la migration doit être silencieuse ; « corriger » l'ordre changerait la fraîcheur perçue du site. L'indépendance des entrées garantit le deploy quotidien même si la sync échoue. La procédure tRPC `organization.deploy` appelle le même service que la tâche. *Alternatives écartées : tâche orchestratrice `jobs:daily` (indirection sans bénéfice), ordre sync-puis-deploy (amélioration visible, contraire à l'iso-legacy — activable plus tard en changeant deux lignes de cron).*
