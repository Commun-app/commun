...
**D4 — Backup : tâche Nitro `db:backup`.** Snapshot SQLite cohérent (API `VACUUM INTO`/backup API de bun:sqlite) poussé via le driver de stockage vers le bucket de l'instance (`backups/YYYY-MM-DD.db`), quotidien via `scheduledTasks`, rétention 30 jours (purge dans la tâche). In-process : aucun sidecar, aucune dépendance Dokploy, et la fonctionnalité profite au self-hosting open source. *Alternatives écartées : backup Dokploy natif (pensé bases serveur, pas fichier SQLite chaud), Litestream (excellent mais une brique de plus à opérer pour 4 clients — réévaluable au SaaS).*

**D5 — Mode ombre par variable d'env.** `COMMUN_JOBS_DISABLED=1` : les tâches planifiées `deploy` et `apidae:sync` retournent `{ skipped: 'shadow-mode' }` sans rien faire (le cron Nitro est cuit dans la config — on ne peut pas le retirer par instance, on neutralise les tâches). Pendant l'observation, le legacy reste l'unique écrivain : ses jobs APIDAE et ses deploys continuent, et le contenu synchronisé arrive dans les instances par la resync (D6). À la bascule : variable retirée → crons actifs aux horaires iso legacy. La sync APIDAE de Commun est déjà validée sur données réelles (passe ot-pertuis du 29/07) ; on peut en plus la déclencher manuellement sur une instance ombre (`/_tasks`) pour comparer son résultat à celui du legacy, sans l'activer en continu.
