...
**D10 — Médias servis publiquement, plus d'URL de LECTURE signée (décision Quentin, 31/07).** Le legacy signait ses URLs de lecture pour 7 jours (`X-Amz-Expires=604800`), alors que ces URLs sont **figées dans un site statique** : un site non reconstruit pendant une semaine perdait toutes ses images, et le cron de déploiement quotidien masquait la panne en permanence. Désormais le préfixe `medias/` est ouvert en lecture par une bucket policy Scaleway (`infra/set-bucket-policy.py`) et `StorageDriver.url()` renvoie une URL directe pour ce préfixe ; l'ÉCRITURE reste signée, et `_seed/` (base migrée), `_backup/` (sauvegardes) et le LISTING du bucket restent privés. Effets : plus de péremption, URLs de ~470 à ~130 caractères (payload ot-pertuis −5,8 Mo — 77 % en étaient des signatures), et le stockage objet — non l'instance — reste dans le chemin du trafic. *Alternative écartée : redirection 302 par l'instance vers une URL signée à la demande — URLs courtes ET révocation immédiate, mais elle place un conteneur unique et non redondé dans le chemin de CHAQUE image ; une panne d'instance, aujourd'hui invisible du public, deviendrait un incident visible chez les quatre clients, à rebours du principe même du site statique.* Contrepartie assumée : la révocation n'est plus automatique — dépublier une page ne coupe plus l'accès au média, il faut supprimer l'objet. Acceptée (ces médias sont déjà publics sur des sites de communes, et les caches des moteurs survivaient de toute façon aux 7 jours). *La promotion/rétrogradation automatique des objets au fil des publications, avec comptage de références, a été explicitement écartée comme disproportionnée — réévaluable en phase 4.*
