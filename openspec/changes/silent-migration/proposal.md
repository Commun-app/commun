...
- **Sauvegarde quotidienne** : tâche Nitro `db:backup` (snapshot SQLite → S3 de l'instance, rétention) — bénéficie aussi au self-hosting open source.
- **Mode ombre** : `COMMUN_JOBS_DISABLED=1` — les tâches planifiées (deploy, apidae:sync) sont des no-op explicites tant que l'instance est en observation ; le legacy reste la seule source d'écriture et de sync APIDAE.
...
- `self-hosting` : ADDED — sauvegarde quotidienne SQLite → S3 (tâche Nitro, rétention).
- `site-deployment` : ADDED — mode ombre (tâches planifiées no-op sous `COMMUN_JOBS_DISABLED`).
- `apidae-sync` : ADDED — mode ombre (idem).
- `admin-app` : ADDED — remise de session (l'admin accepte un token remis par le portail) + base API relative (même origine).

## Impact

- Nouveau `Dockerfile.instance` (build admin avec tokens en secrets de build + API), `apps/portal` (app Nitro légère), scripts d'infra (`infra/` : compose template, resync, sync S3), tâche `db:backup` dans apps/api.
